...
const PaginationItem = styled.a<{ isActive: boolean }>`
  ...
  ${({ isActive }) => (isActive ? `background-color: ${COLOR.bgLightGray};color: ${COLOR.kurlyPurple};` : '')}
`;
...
export default function Pagination({ currentPage, totalPages, onClickPage }: Props) {
  ...
  if (totalPages <= 1 && (pageNumbers.length <= 1 || pageNumbers[-1] === 1)) {
    return null;
  }

  return (
    <Container>
      <PaginationItem isActive={false} onClick={() => setPage(1)}>
        <img src={paginationFirst} alt="처음 페이지로 이동하기 아이콘" />
      </PaginationItem>
      <PaginationItem isActive={false} onClick={() => setPage(currentPage - 1)}>
        <img src={paginationPrev} alt="이전 페이지로 이동하기 아이콘" />
      </PaginationItem>

      {pageNumbers.map((page) => (
        <PaginationItem key={page} isActive={page === currentPage} onClick={() => setPage(page)}>
          {page}
        </PaginationItem>
      ))}
      ...
      <PaginationItem isActive={false} onClick={() => setPage(totalPages)}>
