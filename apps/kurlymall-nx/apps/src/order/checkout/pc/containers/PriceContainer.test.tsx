...
describe('PriceContainer', () => {
  ...
  it('적립금 영역을 볼 수 있다', () => {
    const { container } = renderPriceContainer();
    const { usedFreePoint } = given.price;
    expect(container).toHaveTextContent('적립금');
    expect(container).toHaveTextContent(`${addComma(usedFreePoint)}원`);
  });

  it('컬리캐시 영역을 볼 수 있다', () => {
    const { container } = renderPriceContainer();
    const { usedPaidPoint } = given.price;
    expect(container).toHaveTextContent('컬리캐시');
    expect(container).toHaveTextContent(`${addComma(usedPaidPoint)}원`);
  });

  it('구매시 적립되는 금액을 볼 수 있다', () => {
    const { container } = renderPriceContainer();
