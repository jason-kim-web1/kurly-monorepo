...
describe('CouponContainer test', () => {
  ...
  it('"쿠폰" title 과 쿠폰 사용 문의 button을 볼 수 있다.', async () => {
    renderCouponsContainer();

    await screen.findByText('쿠폰');
    await screen.findByRole('button', { name: '쿠폰 사용 문의 (카카오톡)' });
  });
  ...
  context('멤버스 배너가 등록되어 있으면', () => {
    it('멤버스 배너를 볼 수 있다.', async () => {
      ...
      expect(altText).toEqual(MEMBERS_BANNER.MO_COUPON_LIST);
    });
  });
});
